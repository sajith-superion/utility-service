...
def generate_pptx(data: PptxRequest) -> BytesIO:
    ...
    # 🧩 Preprocess: insert slide breaks between paragraphs
    # This replaces double newlines with a slide separator (---),
    # but skips if a separator already exists nearby.
    ...
    with tempfile.NamedTemporaryFile(suffix=".md", delete=False) as tmp_in:
        tmp_in.write(processed_content.encode("utf-8"))
        tmp_in.flush()
